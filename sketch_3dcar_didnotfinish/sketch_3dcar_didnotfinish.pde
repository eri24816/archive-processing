...
float cx=0;
float cz=0;
float bzps[]=new float[100];
float byps[]=new float[100];
...
void draw(){
 ...
 background(100);
    ...
    if(key=='q'){
      ...
      ty[2]=ty[0];
      deg[2]=deg[0];
      bzps[bn]=sin(radians(bxrot))*sp[2];
      byps[bn]=cos(radians(bxrot))*sp[2];
      tz[2]=50;
    }
    if(key=='w'){
      bxrot-=1;
...
void draw2(){
  ...
  for(int n=0;n<100;n++){
    ...
    by[n]+=byps[n];
    rotateZ(-radians(deg[0]));
    translate(bx[2]-tx[0],by[2]-ty[0],bz[2]-50);
    fill(0);
