...
void setup() {
  ...
  echod=new slide(1400,10,300,1,40);
  echov=new slide(1400,60,300,0,1);
  echos=new slide(1400,110,300,0,100);
  //load(filename);
}
void draw(){
  background(200); //<>//
  ...
  if(selected<100){
    ...
    echos.value=in[selected].echosmooth;
  }
  echod.display();
  echov.display();
  echos.display();
  ...
  if(selected<100){
    ...
    in[selected].echosmooth=echos.value;
  }
...
void load(String name){
  JSONObject json= loadJSONObject("data/"+name+".json");
  JSONArray jin=json.getJSONArray("instrument");;
  ...
  for(int i=0;i<jin.size();i++){
    ...
    in[i].echodelay=o.getInt("echo delay");
    in[i].echovol=o.getFloat("echo volume");
    in[i].echosmooth=o.getFloat("echo smooth");
  ...
  JSONArray newdrum=json.getJSONArray("drum");
  for(int i=0;i<d.m.length;i++){
    JSONArray newarray= newdrum.getJSONArray(i);
    for(int j=0;j<d.m[i].length;j++){
      d.setM(i,j,newarray.getBoolean(j));
    }
  }
...
void save(String name){
  ...
  for(int i=0;i<in.length;i++){
    ...
    newin.setInt("echo delay",in[i].echodelay);
    newin.setFloat("echo volume",in[i].echovol);
    newin.setFloat("echo smooth",in[i].echosmooth);
    jin.setJSONObject(i,newin);
  }
  JSONArray newdrum= new JSONArray();
  for(int i=0;i<d.m.length;i++){
    JSONArray newarray= new JSONArray();
    for(int j=0;j<d.m[i].length;j++){
      newarray.setBoolean(j,d.m[i][j]);
    }
    newdrum.setJSONArray(i,newarray);
  }
  json.setJSONArray("instrument",jin);
  json.setJSONArray("drum",newdrum);
...
void GUI_envelopeSetting(int x,int y,int w,int h){
  ...
  stroke(200,255,100);
  beginShape();
  for(int i=0;i<in[selected]._envelope.length;i++){
    vertex(i*scaleX+x,y+h-in[selected]._envelope[i]*scaleY);
...
void GUI_notes(int x,int y,int w,int h,float px,float py,float scaleX,float scaleY){
  ...
  strokeWeight(10);
  stroke(70,255,200);
  line(x+px+t*scaleX,y,x+px+t*scaleX,y+h);
  strokeWeight(5);
  stroke(70,255,150);
  line(x+px+t*scaleX,y,x+px+t*scaleX,y+h);
  strokeWeight(1);
...
void keyPressed(){
  if(key-49>=0&&key-49<8){
    d.play(key-49);
  }
  text((int)key,700,700);
}
