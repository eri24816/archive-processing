...
void draw(){
  background(200);
  GUI_toneSetting(410,620,600,400);
  GUI_envelopeSetting(1020,620,600,400);
...
void GUI_envelopeSetting(int x,int y,int w,int h){
  ...
  if(mousePressed&&inRect(mouseX,mouseY,x,y,w,h)){
    if((int)((mouseX-x)/scaleX+0.5)<envelope.length&&(int)((mouseX-x)/scaleX)>=0){
      envelope[(int)((mouseX-x)/scaleX+0.5)]=max(min(-(mouseY-y-h),h)/scaleY,0);
    }
  }
}
void mouseDragged(){
...
void GUI_notes(int x,int y,int w,int h,float px,float py,float scaleX,float scaleY){
  ...
  for(int i=0;i<w/scaleX;i++){
    stroke(floor((i*4-px/scaleX+1600+4)%16)<4?50:200);
    strokeWeight(floor((i*4-px/scaleX+1600+4)%16)<4?3:1);
